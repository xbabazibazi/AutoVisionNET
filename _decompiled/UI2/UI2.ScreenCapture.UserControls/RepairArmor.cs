...
public class RepairArmor : UserControl, IServiceControl
{
	private readonly RepairArmors _settings;
	...
	public RepairArmor(WorkflowEngine workflowEngine)
	{
		InitializeComponent();
		_workflowEngine = workflowEngine ?? throw new ArgumentNullException("workflowEngine");
		_logger = Logger.Instance;
		_settings = Settings.Instance.ScreenCapture.RepairArmors;
		LoadSettings();
		SetupEventHandlers();
	}
	...
	public void SaveSettings()
	{
		try
		{
			...
			_logger.LogDebug("RepairArmor ayarları kaydedildi");
		}
		catch (Exception ex)
		{
	...
	private async void checkActive_CheckedChanged(object sender, EventArgs e)
	{
		try
		{
			...
			{
				await _workflowEngine.StartAsync("RepairArmors");
				_logger.LogInformation("RepairArmor servisi başlatıldı");
			}
			else
			{
				await _workflowEngine.StopAsync("RepairArmors");
				_logger.LogInformation("RepairArmor servisi durduruldu");
			}
			_settings.IsActive = checkActive.Checked;
		}
		catch (Exception ex)
	...
	private void InitializeComponent()
	{
		...
		this.labelHeader.Text = "ZIRH TAMİRİ";
		...
		this.toolTip.SetToolTip(this.checkActive, "Zırh tamiri izleme özelliğini etkinleştirir veya devre dışı bırakır");
