...
public class StopMacrosWhenGenieStopped : UserControl, IServiceControl
{
	private readonly StopMacrosOnGenieStop _settings;
	...
	public StopMacrosWhenGenieStopped(WorkflowEngine workflowEngine)
	{
		InitializeComponent();
		_workflowEngine = workflowEngine ?? throw new ArgumentNullException("workflowEngine");
		_logger = Logger.Instance;
		_settings = Settings.Instance.ScreenCapture.StopMacrosOnGenieStop;
		LoadSettings();
		SetupEventHandlers();
	}
	...
	public void SaveSettings()
	{
		try
		{
			...
			_logger.LogDebug("StartGenieAfterTp settings saved");
		}
		catch (Exception ex)
		{
	...
	private async void checkActive_CheckedChanged(object sender, EventArgs e)
	{
		try
		{
			...
			{
				await _workflowEngine.StartAsync("StartGenieAfterTp");
				_logger.LogInformation("StartGenieAfterTp service started");
			}
			else
			{
				await _workflowEngine.StopAsync("StartGenieAfterTp");
				_logger.LogInformation("StartGenieAfterTp service stopped");
			}
			_settings.IsActive = checkActive.Checked;
		}
		catch (Exception ex)
	...
	private void InitializeComponent()
	{
		...
		this.labelHeader.Text = "STOP MACROS WHEN GENIE STOPS";
		...
		this.toolTip.SetToolTip(this.checkActive, "Automatically stop macros when Genie stops");
		...
		base.Name = "StopMacrosWhenGenieStopped";
