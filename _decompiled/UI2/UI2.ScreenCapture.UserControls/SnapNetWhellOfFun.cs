...
public class SnapNetWhellOfFun : UserControl, IServiceControl
{
	private readonly WhellOfFun _settings;
	...
	public SnapNetWhellOfFun(WorkflowEngine workflowEngine)
	{
		InitializeComponent();
		_workflowEngine = workflowEngine ?? throw new ArgumentNullException("workflowEngine");
		_logger = Logger.Instance;
		_settings = Settings.Instance.ScreenCapture.WhellOfFun;
		LoadSettings();
		SetupEventHandlers();
	}
	...
	public void SaveSettings()
	{
		try
		{
			...
			_logger.LogDebug("SnapNetWhellOfFun settings saved");
		}
		catch (Exception ex)
		{
	...
	private async void checkActive_CheckedChanged(object sender, EventArgs e)
	{
		try
		{
			...
			{
				_logger.LogInformation("SnapNetWhellOfFun service started");
			}
			else
			{
				_logger.LogInformation("SnapNetWhellOfFun service stopped");
			}
			_settings.IsActive = checkActive.Checked;
		}
		catch (Exception ex)
	...
	private void InitializeComponent()
	{
		...
		this.labelHeader.Text = "WHELL OF FUN";
		...
		this.toolTip.SetToolTip(this.checkActive, "Enable wheel of fun features");
