...
public class SwapTomahawk : UserControl, IServiceControl
{
	private readonly SettingsManager.ScreenCapture.SwapTomahawk _settings;
	...
	public SwapTomahawk(WorkflowEngine workflowEngine)
	{
		InitializeComponent();
		_workflowEngine = workflowEngine ?? throw new ArgumentNullException("workflowEngine");
		_logger = Logger.Instance;
		_settings = Settings.Instance.ScreenCapture.SwapTomahawk;
		LoadSettings();
		SetupEventHandlers();
	}
	...
	public void SaveSettings()
	{
		try
		{
			...
			_logger.LogDebug("SwapTomahawk settings saved");
		}
		catch (Exception ex)
		{
	...
	private async void checkActive_CheckedChanged(object sender, EventArgs e)
	{
		try
		{
			...
			{
				await _workflowEngine.StartAsync("SwapTomahawk");
				_logger.LogInformation("SwapTomahawk service started");
			}
			else
			{
				await _workflowEngine.StopAsync("SwapTomahawk");
				_logger.LogInformation("SwapTomahawk service stopped");
			}
			_settings.IsActive = checkActive.Checked;
		}
		catch (Exception ex)
	...
	private void InitializeComponent()
	{
		...
		this.labelHeader.Text = "SWAP TOMAHAWK";
		...
		this.toolTip.SetToolTip(this.checkActive, "Enable automatic Tomahawk swapping");
