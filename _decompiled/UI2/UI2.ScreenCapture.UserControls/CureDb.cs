...
public class CureDb : UserControl, IServiceControl
{
	private readonly CureDB _settings;
	...
	public CureDb(WorkflowEngine workflowEngine)
	{
		InitializeComponent();
		_workflowEngine = workflowEngine ?? throw new ArgumentNullException("workflowEngine");
		_settings = Settings.Instance.ScreenCapture.CureDB;
	...
	public void SaveSettings()
	{
		try
		{
			...
			_logger.LogDebug("CureDb ayarları kaydedildi");
		}
		catch (Exception ex)
		{
	...
	private async void checkBoxActive_CheckedChanged(object sender, EventArgs e)
	{
		try
		{
			...
			{
				await _workflowEngine.StartAsync("CureDB");
				_logger.LogInformation("CureDb servisi başlatıldı");
			}
			else
			{
				await _workflowEngine.StopAsync("CureDB");
				_logger.LogInformation("CureDb servisi durduruldu");
			}
			_settings.IsActive = checkBoxActive.Checked;
		}
		catch (Exception ex)
	...
	private void InitializeComponent()
	{
		...
		this.toolTip.SetToolTip(this.checkBoxAlarm, "Cure DB alarm sesini açar/kapatır");
		...
		this.toolTip.SetToolTip(this.checkBoxActive, "Cure DB özelliğini etkinleştirir veya devre dışı bırakır");
