...
public class SnapNetStartGenie : UserControl, IServiceControl
{
	private readonly StartGenie _settings;
	...
	public SnapNetStartGenie(WorkflowEngine workflowEngine)
	{
		InitializeComponent();
		_workflowEngine = workflowEngine ?? throw new ArgumentNullException("workflowEngine");
		_logger = Logger.Instance;
		_settings = Settings.Instance.ScreenCapture.StartGenie;
		LoadSettings();
		SetupEventHandlers();
	}
	...
	public void SaveSettings()
	{
		try
		{
			...
			_logger.LogDebug("SnapNetStartGenie settings saved");
		}
		catch (Exception ex)
		{
	...
	private async void checkActive_CheckedChanged(object sender, EventArgs e)
	{
		try
		{
			...
			{
				_logger.LogInformation("SnapNetStartGenie service started");
			}
			else
			{
				_logger.LogInformation("SnapNetStartGenie service stopped");
			}
			_settings.IsActive = checkActive.Checked;
		}
		catch (Exception ex)
	...
	private void InitializeComponent()
	{
		...
		this.labelHeader.Text = "SNAP NET START GENIE";
